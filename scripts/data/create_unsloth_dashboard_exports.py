...
def encode_summary_q64(frames: list[list[float]]) -> str:
    ...
    trimmed = [f[:width] for f in frames]
    means = [mean(frame[i] for frame in trimmed) for i in range(width)]
    if len(trimmed) > 1:
        deltas = [trimmed[-1][i] - trimmed[0][i] for i in range(width)]
    else:
        deltas = [0.0] * width
    return "m=" + "".join(q64(v) for v in means) + "\nd=" + "".join(q64(v) for v in deltas)
